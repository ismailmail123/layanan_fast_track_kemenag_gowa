...
// ============================================================
// pages/ScanPage.jsx - FIXED: no-cors untuk Apps Script
// ============================================================
import { useState, useRef, useEffect } from "react";
...
// ── Decode QR: support Base64(JSON) dan Base64(nama|lat|lng) ──
const decodeBarcodeFlexibel = (rawText) => {
  // Method 1: JSON mentah
  try {
  ...
  // Method 2: Base64 → JSON (format QR lama)
  try {
    const decoded = atob(rawText);
  ...
  // Method 3: Base64 → nama|lat|lng (format generator baru)
  try {
    ...
    if (parts.length === 3) {
      ...
      if (!isNaN(lat) && !isNaN(lng))
        return { nama: parts[0], latitude: lat, longitude: lng };
    }
  } catch(e) {
    console.error("Error parsing Base64 nama|lat|lng:", e);
  }

  return null;
};

// ── Kirim ke Apps Script dengan no-cors ──────────────────
// no-cors: request tetap sampai ke server, tapi response tidak bisa dibaca.
// Kita generate ID lokal dan asumsikan sukses jika fetch tidak throw error.
const simpanData = async (payload) => {
  const params = new URLSearchParams({
    ...
    localId:      payload.localId   // ID yang kita buat lokal
  });

  // mode: no-cors → tidak ada CORS error, tapi response adalah "opaque" (tidak bisa dibaca)
  await fetch(`${APPS_SCRIPT_URL}?${params.toString()}`, {
    method: "GET",
    mode: "no-cors"
  });
  // Jika baris di atas tidak throw, request berhasil dikirim ke server
  return true;
};
...
export default function ScanPage() {
  ...
  const prosesRef  = useRef(false);   // guard: proses sekali saja per scan
  ...
  const mulaiScan = async () => {
    if (!nama.trim()) { setStatus({ tipe: "error", pesan: "Nama petugas wajib diisi sebelum scan!" }); return; }
    setHasilScan(null);
    setStatus({ tipe: "info", pesan: "📡 Mengambil posisi GPS Anda..." });
    ...
    try {
      ...
      setStatus({ tipe: "info", pesan: `✅ GPS aktif (akurasi ±${Math.round(posisi.accuracy)} m).\nArahkan kamera ke QR Code titik patroli.` });
      setScanning(true);
  ...
  const prosesHasilScan = async (rawText) => {
    ...
    if (!dataTitik) {
      setStatus({ tipe: "error", pesan: "QR Code tidak valid!\nPastikan Anda scan QR Code resmi yang dibuat dari menu Generator QR." });
      setLoading(false);
      return;
    }
    ...
    if (jarak > JARAK_MAKS) {
      setStatus({
        tipe: "warning",
        pesan: `JARAK TERLALU JAUH!\n\nAnda berada ${jarakBulat} meter dari titik "${dataTitik.nama}".\nMaksimal yang diizinkan: ${JARAK_MAKS} meter.\n\nPindah lebih dekat ke titik QR Code, lalu scan ulang.`
      });
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // Dengan no-cors kita tidak bisa baca response, tapi jika tidak throw = terkirim
      setHasilScan({
        namaTitik: dataTitik.nama,
        jarak:     jarakBulat,
        jam:       formatWaktu(now),
      ...
      setStatus({ tipe: "error", pesan: `Gagal mengirim data.\nError: ${err.message}\n\nCek koneksi internet Anda.` });
    }
    ...
    <div>
      ...
      {hasilScan && (
        <div style={{ background:"rgba(34,197,94,0.08)", border:"1px solid rgba(34,197,94,0.25)", borderRadius:16, padding:20, marginTop:16 }}>
          <div style={{ fontSize:11, fontWeight:700, color:"#4ade80", letterSpacing:1, textTransform:"uppercase", marginBottom:12 }}>✅ Tercatat di Spreadsheet</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
          ...
          <div style={{ fontSize:10, color:"#475569", marginTop:12, textAlign:"center", fontFamily:"monospace" }}>
            {hasilScan.id}
          </div>
          {/* Catatan no-cors */}
          <div style={{ marginTop:10, fontSize:11, color:"#64748b", textAlign:"center", lineHeight:1.5 }}>
            Data terkirim ke Google Sheets.{"\n"}Cek tab "Hari Ini" untuk verifikasi.
          </div>
        </div>
      )}
